...
export function Calendar() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api
        .get(
          `/api/calendar-overrides?year=${month.getFullYear()}&month=${month.getMonth() + 1}`,
        )
        .then(setOverrides)
        .catch(() => setOverrides({})),
  ...
  function predominantMood(moods) {
    ...
    const isDivergent =
      new Set(moods).size > 1 && sorted[0][1] === sorted[1]?.[1];
    ...
    <div className="calendar-page">
      <div className="calendar-title-row">
        ...
        <ScreenTitle
          ...
          sub={
            language === "pt"
              ? "Cada dia recebe a cor do humor que o definiu. Dias divididos são marcados."
              : "Each day takes the colour of its defining mood. Mixed days are marked."
          }
        />
        <button
          type="button"
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                ...
                >
                  {dayNumber}
                  {result?.isDivergent && (
                    <span className="calendar-divergent-mark" />
                  )}
                </button>
              );
            })}
            ...
            <div className="day-detail-list">
              {dayDetail.entries.length === 0 && (
                <p className="sheet-sub">
                  {language === "pt"
                    ? "Nenhuma entrada com texto nesse dia."
                    : "No entries with text on this day."}
                </p>
              )}
              {dayDetail.entries.map((entry) => (
                <div key={entry.id} className="day-detail-entry">
                  <div className="day-detail-entry-head">
                    ...
                    <span className="day-detail-moment">
                      {t.moments[entry.moment]?.label || entry.moment}
                    </span>
                  ...
                  {entry.text_content && (
                    <p className="day-detail-text">{entry.text_content}</p>
                  )}
      ...
      {tieBreakDay && (
        <div className="modal-backdrop" onClick={() => setTieBreakDay(null)}>
          <div className="modal glass" onClick={(e) => e.stopPropagation()}>
            ...
            <h2>
              {language === "pt"
                ? "Esse dia puxou pra dois lados."
                : "This day pulled in two directions."}
            </h2>
            <p className="sheet-sub">
              {language === "pt"
                ? "Qual representa melhor esse dia? Você decide."
                : "Which one best represents it? You decide."}
            </p>
            <div className="sheet-mood-list">
              {[...new Set(tieBreakDay.moods)].map((m) => (
                <button
                  key={m}
                  type="button"
                  onClick={() => resolveTieBreak(tieBreakDay.key, m)}
                >
                  <span className={`legend-dot mood-${m}`} />
                  {t.moods[m]}
                </button>
